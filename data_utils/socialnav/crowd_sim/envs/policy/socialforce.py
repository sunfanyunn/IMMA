...
class CentralizedSocialForce(SocialForce):
    ...
    def predict(self, state):
        ...
        for i, agent_state in enumerate(state):
            ...
            assert i != leader
            velocity = np.array((state[leader].px - agent_state.px, state[leader].py - agent_state.py))
            speed = np.linalg.norm(velocity)
            pref_vel = velocity / speed if speed > 1 else velocity
            pref_vel = pref_vel * agent_state.v_pref

            sf_state.append((agent_state.px, agent_state.py, pref_vel[0], pref_vel[1],
                             agent_state.gx, agent_state.gy))

        sim = socialforce.Simulator(np.array(sf_state), delta_t=self.time_step, initial_speed=self.initial_speed,
                                    v0=self.v0, sigma=self.sigma)
        self.forces = sim.step()
