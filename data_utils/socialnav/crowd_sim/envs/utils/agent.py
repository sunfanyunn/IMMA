...
class Agent(object):
    ...
    def get_next_observable_state(self, action, boundary=None):
        ...
        pos = self.compute_position(action, self.time_step, boundary)
        next_px, next_py = pos
    ...
    def compute_position(self, action, delta_t, boundary=None):
        self.check_validity(action)
        if self.kinematics == 'holonomic':
        ...
        if boundary and np.sqrt(px**2 + py**2) + self.radius > boundary:
            fa = np.sqrt(px**2 + py**2)
            px, py = (boundary - self.radius) * px / fa, (boundary - self.radius) * py / fa
        return px, py

    def step(self, action, boundary=None):
        """
        Perform an action and update the state
        """
        self.check_validity(action)
        pos = self.compute_position(action, self.time_step, boundary=boundary)
        self.px, self.py = pos
        if self.kinematics == 'holonomic':
            self.vx = action.vx
            self.vy = action.vy
