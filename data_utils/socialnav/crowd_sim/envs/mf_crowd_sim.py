...
class MFCrowdSim(gym.Env):
    ...
    def configure(self, config):
        ...
        self.obs_frames = config.env.obs_frames
        ...
        if self.centralized_planning:
            ...
            config.humans.policy = 'centralized_' + human_policy
    ...
    def generate_human(self, human=None, center=False):
        if human is None:
        ...
        if self.current_scenario == 'circle_crossing':
            ...
            # if self.centralized_planner.name == 'LFORCA':
            if self.centralized_planner.name == 'LFORCA' or self.centralized_planner.name == 'SocialForce':
                while True:
                    angle = np.random.random() * np.pi * 2
                    # add some noise to simulate all the possible cases robot could meet with human
                    px_noise = 0 #(np.random.random() - 0.5) #* human.v_pref
                    py_noise = 0 #(np.random.random() - 0.5) #* human.v_pref
                    px = (self.circle_radius - human.radius) * np.cos(angle) + px_noise
                    py = (self.circle_radius - human.radius) * np.sin(angle) + py_noise
                    collide = False
                    ...
                    if not collide:
                        break
                angle = np.random.random() * np.pi * 2
                leng = np.random.rand() * (self.circle_radius - human.radius)
                gx = leng * np.cos(angle)
                gy = leng * np.sin(angle)
                human.set(px, py, gx, gy, 0, 0, 0)
                # human.set(px, py, 0, 0, 0, 0, 0)
                human.belief = 0
            else:
                quadrants = [0, 0.125, 0.25, 0.375, 0.5, 0.625, 0.75, 0.875, 1.]
                gt_leng = (self.circle_radius - human.radius)
                while True:
                    init_leng = np.random.uniform(0, self.circle_radius//2)
                    qidx = np.random.choice(np.arange(8))
                    lb = min(quadrants[qidx], quadrants[qidx+1])
                    ub = max(quadrants[qidx], quadrants[qidx+1])
                    angle = np.random.uniform(lb, ub) * np.pi * 2
                    # add some noise to simulate all the possible cases robot could meet with human
                    px_noise = 0 #(np.random.random() - 0.5) #* human.v_pref
                    py_noise = 0 #(np.random.random() - 0.5) #* human.v_pref
                    px = init_leng * np.cos(angle) + px_noise
                    py = init_leng * np.sin(angle) + py_noise
                    collide = False
                    for agent in [self.robot] + self.humans:
                        min_dist = human.radius + agent.radius + self.discomfort_dist
                        if norm((px - agent.px, py - agent.py)) < min_dist or \
                ...
                qidx = (qidx+np.random.randint(3, 7))%8
                # lb = min(quadrants[qidx], quadrants[qidx+1])
                # ub = max(quadrants[qidx], quadrants[qidx+1])
                # angle = np.random.uniform(lb, ub) * np.pi * 2
                angle = quadrants[qidx] * np.pi * 2
                gx = gt_leng * np.cos(angle)
                gy = gt_leng * np.sin(angle)
                human.set(px, py, gx, gy, 0, 0, 0)
                # human.set(px, py, 0, 0, 0, 0, 0)
                human.belief = qidx
    ...
    def reset(self, phase='test', test_case=None):
        ...
        if self.centralized_planning:
            ...
            self.centralized_planner.n_humans = len(self.humans)
            self.centralized_planner.reset()
            self.centralized_planner.obs_frames = 1

        NAN = ObservableState(0,0,0,0,0)
        self.obs = [[NAN for i in range(len(self.humans))] for _ in range(self.obs_frames)]
        self.states = list()
        ...
        if self.centralized_planner.name == 'LFORCA' or self.centralized_planner.name == 'SocialForce':
            leaders = self.centralized_planner.leaders.copy()
            num_humans = len(leaders)
            normalized_A = torch.zeros((num_humans, num_humans))
            for follower, leader in enumerate(leaders):
                normalized_A[follower, leader] += 1
            self.graph = normalized_A
            assert not self.graph.requires_grad
        else:
            num_goals = 8
            n_humans = self.centralized_planner.n_humans
            self.graph = torch.zeros((n_humans, num_goals))
            for i in range(n_humans):
                self.graph[i, self.humans[i].belief] = 1

        return self.multistep_obs(ob)
    ...
    def step(self, action, update=True):
        ...
        for i in range(human_num):
            # if self.centralized_planner.leaders[i] == i:
                # if math.sqrt((self.humans[i].px - self.humans[i].gx)**2 + (self.humans[i].py - self.humans[i].gy)**2) < 0.1:
                    # angle = np.random.random() * np.pi * 2
                    # leng = np.random.rand() * (self.circle_radius - human.radius)
                    # gx = leng * np.cos(angle)
                    # gy = leng * np.sin(angle)
        ...
        if update:
            ...
            boundary = self.circle_radius
            # update all agents
            self.robot.step(action, boundary)
            for human, action in zip(self.humans, human_actions):
                human.step(action, boundary)
        ...
        return self.multistep_obs(ob), reward, done, info

    def multistep_obs(self, ob):
        self.obs.append(ob)
        return (self.obs[-self.obs_frames:], self.graph)
    ...
    def render(self, mode='video', output_file=None):
        ...
        if mode == 'traj':
            ...
            # add bounary
            boundary = plt.Circle((0,0), self.circle_radius, fill=False, color='black')
