...
from crowd_nav.policy.multistep_graph_model import MultistepRGL
from crowd_nav.policy.value_estimator import ValueEstimator


class MultistepModelPredictiveRL(Policy):
    def __init__(self):
        super().__init__()
    ...
    def configure(self, config):
        ...
        if self.linear_state_predictor:
            ...
            if self.share_graph_model:
                ...
                # graph_model1 = MultistepRGL(config, self.robot_state_dim, self.human_state_dim)
                ...
                graph_model2 = MultistepRGL(config, self.robot_state_dim, self.human_state_dim)
    ...
    def predict(self, states):
        ...
        states, graph = states[0], states[1]
        state = states[-1]
        ...
        if self.phase == 'train' and probability < self.epsilon:
            ...
            if self.do_action_clip:
                state_tensors = self.transform(states)
                state_tensors = (state_tensors[0].unsqueeze(0).to(self.device), state_tensors[1].unsqueeze(0).to(self.device))
                action_space_clipped = self.action_clip((state_tensors, graph), self.action_space, self.planning_width)
            else:
            ...
            for action in action_space_clipped:
                ...
                if graph is not None:
                    next_state = self.state_predictor((state_tensors, graph.unsqueeze(0).to(self.device)), action)
                else:
                    next_state = self.state_predictor((state_tensors, None), action)
                max_next_return, max_next_traj = self.V_planning(next_state, graph, self.planning_depth, self.planning_width)
                reward_est = self.estimate_reward(state, action)
                value = reward_est + self.get_normalized_gamma() * max_next_return
                if value > max_value:
                    max_value = value
                    max_action = action
                    max_traj = [(state_tensors, action, reward_est)] + max_next_traj
            if max_action is None:
                raise ValueError('Value network is not well trained.')

        if self.phase == 'train':
            self.last_state = self.transform(states)
        else:
            self.traj = max_traj
    ...
    def action_clip(self, state, action_space, width, depth=1):
        ...
        for action in action_space:
            next_state_est = self.state_predictor((state, graph.unsqueeze(0)), action)
            next_return, _ = self.V_planning(next_state_est, graph, depth, width)
            reward_est = self.estimate_reward(state, action)
    ...
    def V_planning(self, state, graph, depth, width):
        ...
        if graph is not None:
            current_state_value = self.value_estimator((state, graph.unsqueeze(0)))
        else:
            current_state_value = self.value_estimator((state, None))
        if depth == 1:
            return current_state_value, [(state, None, None)]

        if self.do_action_clip:
            action_space_clipped = self.action_clip((state, graph), self.action_space, width)
        else:
            action_space_clipped = self.action_space
        ...
        for action in action_space_clipped:
            next_state_est = self.state_predictor((state, graph.unsqueeze(0)), action)
            reward_est = self.estimate_reward(state, action)
            next_value, next_traj = self.V_planning(next_state_est, graph, depth - 1, self.planning_width)
    ...
    def get_matrix_A(self):
        return self.state_predictor.graph_model.A
    ...
    def transform(self, states):
        ...
        """
        Take list of JointStates to tensors
        ...
        for state in states:
            ...
            human_states_tensor = torch.Tensor([human_state.to_tuple() for human_state in state.human_states]). \
                to(self.device)
            robot_state_tensors.append(robot_state_tensor)
            human_states_tensors.append(human_states_tensor)

        return torch.stack(robot_state_tensors), torch.stack(human_states_tensors)
