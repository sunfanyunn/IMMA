...
class MultistepRGL(nn.Module):
    def __init__(self, config, robot_state_dim, human_state_dim):
        ...
        self.wr_dims = wr_dims
        self.wh_dims = wh_dims
        self.w_r = mlp(config.obs_frames*robot_state_dim, wr_dims, last_relu=True)
        self.w_h = mlp(config.obs_frames*human_state_dim, wh_dims, last_relu=True)
        # self.lstm_r = nn.LSTM(wr_dims[1], wr_dims[1], batch_first=True)
        # self.lstm_h = nn.LSTM(wh_dims[1], wh_dims[1], batch_first=True)
    ...
    def forward(self, states):
        ...
        # states: [robot_states, human_states]
        # robot_states.shape (batch_size, obs_frames, 1, robot_state_dim)
        # human_states.sahpe (batch_size, obs_frames, num_humans, human_state_dim)
        assert states[0].shape[0] == states[1].shape[0]
        batch_size = states[1].shape[0]
        num_humans = states[1].shape[2]
        robot_state = states[0].view(batch_size, 1, -1)
        human_states = states[1].permute(0, 2, 1, 3)
        ...
        if not self.layerwise_graph:
            ...
            # normalized_A = 0.5 * (graph + self.compute_similarity_matrix(X))
