...
class StatePredictor(nn.Module):
    ...
    def forward(self, state, action, detach=False):
        ...
        # (batch_size, obs_frames, # of agents, feature_size)
        ...
        # state is a tuple/list of two tensors

        state_embedding = self.graph_model((state, graph))
        if detach:
        ...
        if action is None:
            ...
            obs_frames = state[0].shape[1]
            next_robot_state = self.compute_next_state(state[0][:, -1, ...], action)
            next_robot_state = torch.cat([state[0][:, 1:, ...],
                                          next_robot_state.unsqueeze(1)], dim=1)

        next_human_states = self.human_motion_predictor(state_embedding)[:, 1:, :]
        next_human_states = torch.cat([state[1][:, 1:, ...],
                                       next_human_states.unsqueeze(1)], dim=1)
