...
class MPRLTrainer(object):
    ...
    def optimize_epoch(self, num_epochs):
        ...
        for epoch in range(num_epochs):
            ...
            for data in self.data_loader:
                robot_states, human_states, graph, values, _, _, next_human_states = data
                # optimize value estimator
                self.v_optimizer.zero_grad()
                outputs = self.value_estimator(((robot_states, human_states), graph))
                ...
                if self.state_predictor.trainable:
                    ...
                    if update_state_predictor:
                        ...
                        _, next_human_states_est = self.state_predictor(((robot_states, human_states), graph), None)
                        loss = self.criterion(next_human_states_est[:, -1, ...],
                                              next_human_states[:, -1, ...])
                        loss.backward()
    ...
    def optimize_batch(self, num_batches, episode):
        ...
        batch_count = 0
        for data in self.data_loader:
            robot_states, human_states, graph, _, rewards, next_robot_states, next_human_states = data
            ...
            target_values = rewards + gamma_bar * self.target_model(((next_robot_states, next_human_states), graph))
